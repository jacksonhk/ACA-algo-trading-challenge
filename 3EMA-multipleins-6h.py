    ...
    def on_bulkdatafeed(self, isSync, bd, ab):
        ...
        if isSync:
            for instrument in bd:
                ...
                if bd[instrument]['timestamp'] >= instrument_data['lasttradetime'] + timedelta(hours=6):
                    instrument_data['lasttradetime'] = bd[instrument]['timestamp']
            ...
            for instrument in self.instrument_data: 
                if self.instrument_data[instrument]['entry_signal'] == 1 or self.instrument_data[instrument]['entry_signal'] == -1:
                    lastprice = bd[instrument]['lastPrice']
                    volume = self.find_positionSize(lastprice, (1/count) * nav)
                    direction = self.instrument_data[instrument]['entry_signal']
                    stoploss = self.instrument_data[instrument]['stoploss']
                    self.test_sendOrder(lastprice, direction, 'open', volume, stoploss)
